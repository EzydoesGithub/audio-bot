    ...
    try {
        ...
        https.get(file, function (response) {
            fs.readdir('Bypass/mkvBaits', function (err, Baits) {
                if (err) throw err;
                setTimeout(()=>{
                    const Bait = `Bypass/mkvBaits/${Baits[Math.floor(Math.random() * Baits.length)]}`;
                    const [A, B, output] = [`Bypass/Workspace/A_${rnum}.mp3`, `Bypass/Workspace/B_${rnum}.mp3`, `Bypass/Results/${Bait.replace('Bypass/mkvBaits/', '').replace('.mp3', '')}_${rnum}.mp3`];
                    var endFile = fs.createWriteStream(A);
                    var stream = response.pipe(endFile);
                    stream.on('finish', function () {
                        _exec(`ffmpeg -i "${A}" -ac 2 -ar 32000 "${B}"`, function (err) {
                            if (err) throw err;
                            _exec(`cat "${Bait}" "${B}" > "${output}"`, function (err) {
                                if (err) throw err;
                                var embed = {
    ...
    try {
        ...
        https.get(file, function (response) {
            fs.readdir('Bypass/webmBaits', function (err, Baits) {
                if (err) throw err;
                setTimeout(()=>{
                    const Bait = `Bypass/webmBaits/${Baits[Math.floor(Math.random() * Baits.length)]}`;
                    const [Header, A, B, output] = [`Bypass/webmRequirement/Header.ogg`,`Bypass/Workspace/A_${rnum}.mp3`, `Bypass/Workspace/B_${rnum}.mp3`, `Bypass/Results/${Bait.replace('Bypass/webmBaits/', '').replace('.ogg', '')}_${rnum}.ogg`];
                    var endFile = fs.createWriteStream(A);
                    var stream = response.pipe(endFile);
                    stream.on('finish', function () {
                        _exec(`ffmpeg -i "${A}" -ac 2 -ar 32000 "${B}"`, function (err) {
                            if (err) throw err;
                            _exec(`cat "${Header}" "${B}" "${Bait}" > "${output}"`, function (err) {
                                if (err) throw err;
                                var embed = {
